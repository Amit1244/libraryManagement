...
export const addBooks = async (body, files) => {
    try {
        body.image = files && files.map((ele) => {
            return {
                _id: new ObjectId(),
...
export const findBook = async (body) => {
    try {
        const response = await Book.findOne({ name: { $regex: new RegExp(body.name, "i") } })
        return commonSendResponse(response)
    } catch (error) {
        return error
    }
...
export const getBooks = async (limit, skip, category, name) => {
    try {
        ...
        obj.isDeleted = 0;
        const response = await Book.find(obj).select({ isDeleted: 0 }).skip(skip).limit(limit)
...
export const editBookDetails = async (file, imageId, id) => {
    try {
        const response = await Book.findOneAndUpdate({ _id: id, "image._id": new ObjectId(imageId) }, { "image.$.fileName": file.filename })
        return commonSendResponse(response)
    } catch (error) {
        return error
    }
...
export const editCategorytails = async (body) => {
    try {
        const response = await Category.findByIdAndUpdate({ _id: body.id }, { name: body.name })
        return commonSendResponse(response)
    } catch (error) {
        return error
    }
